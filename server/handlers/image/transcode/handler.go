...
func saveImageToCache(image *vips.ImageRef, imagePath, prefix, imageHash string, width, height int) ([]byte, error) {
	baseDirectory := filepath.Join(imagePath, prefix, imageHash)
	filePath := filepath.Join(baseDirectory, fmt.Sprintf("%dx%d.webp", width, height))

	export, _, webPExportErr := image.ExportWebp(vips.NewWebpExportParams())
	if webPExportErr != nil {
		return nil, fmt.Errorf("failed to export image to webp: %w", webPExportErr)
	}

	cacheDirErr := os.MkdirAll(baseDirectory, 0o755)
	if cacheDirErr != nil {
		return nil, fmt.Errorf("failed to create cache directory: %w", cacheDirErr)
	}

	fileWriteErr := os.WriteFile(filePath, export, helpers.BaseFilePermissions)
	if fileWriteErr != nil {
		return nil, fmt.Errorf("failed to write image to cache: %w", fileWriteErr)
	}
...
func (handler *ImageHandler) HTTPHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if imageQuery.URL == "" || !strings.HasPrefix(imageQuery.URL, "/") {
		log.Err(errInvalidURL).Str("url", imageQuery.URL).Msg("invalid transcode request")
	...
	if imageQuery.Height != 0 && imageQuery.Width != 0 {
		...
		_, imageStatErr := os.Stat(imagePath)

		switch {
		case errors.Is(imageStatErr, nil):
			log.Debug().Str("path", imagePath).Msg("Cached image found")

			isCached = true
		case errors.Is(imageStatErr, os.ErrNotExist):
	...
	if imageQuery.Height != 0 && imageQuery.Width != 0 {
		...
		if shouldCache {
			export, saveImageErr := saveImageToCache(
				image,
				handler.imageCachePath,
				prefix,
				imageHash,
				imageQuery.Width,
				imageQuery.Height)
			if saveImageErr != nil {
				log.Err(saveImageErr).Msg("Failed to save image to cache")
				http.Error(writer, "Failed to save image to cache", http.StatusInternalServerError)
	...
	if err != nil {
		log.Err(err).Msg("Failed to write image to response")
		http.Error(writer, "Failed to write image to response", http.StatusInternalServerError)
	}
