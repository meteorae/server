...
func getImages(result *client.ShowResult, item sdk.Item) (sdk.Art, sdk.Posters) {
	...
	if len(sortedArt) > 0 {
		for index, art := range sortedArt {
			...
			showArt.Items = append(showArt.Items, sdk.ItemImage{
				External:  true,
	...
	if len(sortedPosters) > 0 {
		for index, poster := range sortedPosters {
			...
			showPosters.Items = append(showPosters.Items, sdk.ItemImage{
				External:  true,
				Provider:  "tv.meteorae.agents.fanarttv",
...
func GetMetadata(item sdk.Item) (sdk.Item, error) {
	...
	if showItem, ok := item.(sdk.TVShow); ok {
		log.Debug().
			Str("identifier", "tv.meteorae.agents.fanarttv").
			Uint("item_id", showItem.ID).
			Str("title", showItem.Title).
			Msgf("Getting metadata for movie")

		// Get the TMDb ID
		tmdbID, getTmdbIDErr := getTmdbID(item)
		if getTmdbIDErr != nil {
			log.Err(getTmdbIDErr).Msgf("Failed to get TMDb ID for %d", showItem.ID)
		...
		if tmdbID != 0 {
			...
			showImages, getShowErr := fanartClient.GetShowImages(ctx, fmt.Sprintf("%d", tmdbID))
			if getShowErr != nil {
				log.Err(getShowErr).Msgf("Failed to get show images for %d", tmdbID)

				return nil, fmt.Errorf("failed to get show images: %w", getShowErr)
			}

			showArt, showPosters := getImages(showImages, item)
